  ...
  backup_brain: {
    ...
    properties: {
      ...
      workspaceDir: {
        ...
        minLength: 1,
        description:
          'Optional explicit OpenClaw workspace directory containing AGENTS.md/IDENTITY.md/SOUL.md/TOOLS.md/USER.md',
      },
    },
    required: ['trigger'],
    additionalProperties: false,
...
export async function register(api: PluginAPI): Promise<void> {
  const configGetter = (api as unknown as { getConfig?: () => SentinelPluginConfig }).getConfig;
  ...
  const notConfiguredMessage =
    "Sentinel API key is not configured. Set plugin config 'apiKey' or SENTINEL_API_KEY, then reload plugins.";
    ...
    : [
        {
          ...
          description:
            "Saves important information to Sentinel Cloud. Trigger this if the user says 'Remember that...' or provides critical config/preferences.",
  ...
  const registerToolFn =
    (api as unknown as { registerTool?: (tool: ToolRegistration) => void }).registerTool ??
    (api as unknown as { tool?: { register?: (tool: ToolRegistration) => void } }).tool?.register;
